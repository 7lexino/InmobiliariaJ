...
router.get('/ultima_por_contrato/:noContrato', async (req, res) => {
    const transaccion = await Transaccion.find({ noContrato: req.params.noContrato}, null, { limit: 1 }).sort({createdAt:-1, fecha:-1})
...
router.get('/transacciones_empresa', async (req, res) => {
    const transacciones = await Transaccion.find({ $or: [{adjuntoId: "transferencia", adjuntoId: "cheque", adjuntoId: "deposito"}], noContrato: 0, $or: [{tipo: "abono"}, {tipo: "egreso"}] }, {}, { sort: { 'fecha': 1, 'createdAt': 1 } });
    res.status(200).json(transacciones);
});
